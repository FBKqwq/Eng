...
The fixed business pages are `home`, `monitor`, `diagnosis`, `results`, and `system`.

**Goulingming 页面目录（新增页面维护位置）**

所有新建业务页面统一维护在以下路径：

- `src/views/goulingming/`：每个页面一个子目录，子目录名即路由路径名
- `src/views/goulingming/com/`：该目录下所有页面共享的通用组件（可直接复用 `src/components/` 下已有组件）
- 页面内局部使用的展示组件，放在页面子目录内（如 `src/views/goulingming/foo/components/`）

Goulingming 页面需额外注册路由和侧边栏菜单项后方可访问。
...
**Goulingming 页面开发流程**（`src/views/goulingming/` 下新建页面）：

1. 在 `src/views/goulingming/` 下创建以路由路径命名的子目录（如 `my-page/`）。
2. 在该子目录下编写页面入口 `index.vue`，引用 `src/views/goulingming/com/` 下的共享组件或 `src/components/` 下已有组件。
3. 页面级局部组件放在 `my-page/components/` 下。
4. 在 `src/router/index.js` 中新增路由并导入对应组件。
5. 在 `src/layout/menu.js` 的 `menuTree` 中新增菜单项。
6. 如需独立 API，在 `src/api/` 下新增或扩展对应文件。
7. 完成后回答：涉及文件、为什么这样改、代码摘要、后端接口前提、未完成边界。
...
Goulingming 页面任务另需说明：路由是否已在 `router/index.js` 注册、菜单是否已在 `menu.js` 添加。
