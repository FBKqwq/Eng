...
description: Guides backend development inside the location/backend/ directory of the ELK + Kafka + LangGraph e-commerce log analysis and intelligent diagnosis project. Enforces strict module boundaries (api / schemas / services / tasks), mandates rule-engine-first diagnosis routing before LangChain/LangGraph, forbids leaking backend duties to frontend or infrastructure directories, and standardizes output format from layer to files to reasoning to code to status. Maintains the yc/ directory under app/services/ for anomaly prediction and early warning enhancement services. Use when the user works on backend code, adds APIs, writes Kafka producers, Elasticsearch queries, diagnosis services, log simulation tasks, or discusses backend architecture, schemas, services, or LangGraph orchestration inside this project.
---
