...
## 5. 待开发功能清单（P0-P3）
- P0：接入真实日志上下文（经 ES service 检索）后再输出诊断，区分占位与真实链路。
...
|---|---|---|---|---|---|
| Diagnosis Service | 可用但需完善 | 2026-05-11 | codex | 中 | 核心为规则+占位返回，LLM/图未接通 |
...
| 方向 | 设计愿景 | 当前状态 | 后续动作 |
|---|---|---|---|
| 智能诊断闭环 | 规则分流 → 上下文检索 → LLM/图式分析 → 可解释结构化结果 | 规则可用，整体仍为占位说明与固定建议 | 先接 ES 上下文，再按需接入 LangChain/LangGraph |
