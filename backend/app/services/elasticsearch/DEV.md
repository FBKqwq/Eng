...
- `search_logs()`：基于 `LogQueryRequest` 构建 Elasticsearch bool 查询，支持分页、排序、关键词、时间范围和常见字段过滤；ES 不可用时返回稳定错误结构。
- `search_recent_context()`：为诊断服务提供受控的相关日志上下文查询入口，避免诊断模块直接拼 ES DSL。

## 5. 待开发功能清单（P0-P3）
- P0：在本机 ELK 服务启动后完成真实数据联调和查询结果校验。
- P1：补齐聚合查询与时间窗口统计能力。
- P2：根据实际 ES mapping 优化 `.keyword` 字段、排序字段和高亮策略。
- P3：缓存热门查询与慢查询诊断能力。

## 6. 模块状态表
| 模块名称 | 当前状态 | 最近修改时间 | 最近修改人/agent | 风险等级 | 备注 |
|---|---|---|---|---|---|
| Elasticsearch Service | 可用但需联调 | 2026-05-18 | codex | 中 | 已接入真实 search；当前环境 ES 未启动时会返回稳定错误结构 |
...
| 方向 | 设计愿景 | 当前状态 | 后续动作 |
|---|---|---|---|
| 高可用日志查询 | 结构化、高性能、可扩展查询能力 | 已有真实 ES search 与稳定错误返回 | 结合实际索引 mapping 继续优化字段类型、聚合和高亮 |
...
| 2026-05-18 | 日志查询从占位升级为真实 Elasticsearch search | `app/services/elasticsearch/log_query_service.py` | 支持 bool 查询、分页、排序、结果转换和 ES 不可用错误结构 | 需在 ELK 容器启动后用真实索引验证命中数据 |
