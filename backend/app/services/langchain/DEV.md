...
> **M7 收口说明**：M7 完成后，LangChain 层五类 Prompt、四条主 Chain（report / diagnosis / alert / relation）均已实装；**M7 完成即总体规划（M1~M7）全部收口**（测试收口见 M7-09）。
...
- **`prompts`**：report、diagnosis、evidence_summary、alert 四套可用模板；`get_prompt(name)` 未知名称返回空串。
...
- **`RELATION_PROMPT`（M7-01）**：`prompts.get_prompt("relation")` 返回完整模板；输入占位符 `{evidence_package}`（JSON 证据包）；约束 LLM 仅输出 `relations` 数组 JSON，证据不足时输出空数组。
- **`RelationChainOutput` / `RelationItem`（M7-02）**：
  - `RelationItem`：`relation_type`（`temporal_correlation` | `causal_hypothesis` | `co_occurrence`）、`description`、`entities`、`confidence`（0~1 钳制）、`evidence_refs`。
  - `RelationChainOutput`：`relations: list[RelationItem]`。
- **`relation_chain`（M7-03）**：`discover_relations(evidence_package)` 已实现 LLM 结构化路径 + 降级。

#### `discover_relations` 输入/输出契约

| 方向 | 字段 | 说明 |
|---|---|---|
| **输入** | `evidence_package` | `dict`；兼容 `build_evidence_package` 完整返回值（含内层 `evidence_package`）或直接传入内层证据包（含 `summary` / `grouped` / `samples` / `metrics`） |
| **输出** | `ok` | 恒为 `True`（降级亦不阻断下游） |
| **输出** | `degraded` | LLM 不可用、解析失败或异常时为 `True`；成功路径为 `False` |
| **输出** | `relations` | `list[dict]`，每项为 `RelationItem.model_dump()`；降级时为空列表 `[]` |

#### `discover_relations` 降级策略

1. `llm_manager.is_llm_available()` 为 False → 空 `relations`，`degraded: true`。
2. `prompts.get_prompt("relation")` 缺失 → 同上。
3. `invoke_structured("relation", ...)` 失败或 `RelationChainOutput` 校验失败 → 记录 info/exception 后降级。
4. 降级不抛异常，保证定时子图 `analyze_relations` 节点可继续执行 `generate_report`。

## 5. 待开发功能清单

- P3：关系发现单测纳入 `test_m7_enhancements`（M7-09 负责）。
...
| `prompts` | 已实现 | 2026-06-22 | Prompt Agent (M7-01) | 低 | 五类模板均已落地 |
...
| `chain_schemas` | 已实现 | 2026-06-22 | Schema Agent (M7-02) | 低 | 含 Report / Diagnosis / Relation 链层模型 |
...
| `relation_chain` | 已实现 | 2026-06-22 | 关系发现 Chain Agent (M7-03) | 低 | LLM + 空列表降级；由定时子图 `analyze_relations` 调用 |
| `alert_chain` | 已实现 | 2026-06-22 | elk-backend-agent (M6-01) | 低 | LLM + alert_type/service/severity 模板降级 |
| LangChain 层（整体） | **M3 + M6 + M7 主链路完成** | 2026-06-22 | 文档 Agent (M7-10) | 低 | 五 Prompt + 四 Chain 均已实装 |
...
| `relation_chain` | `invoke_structured("relation")` 成功 → `degraded: false` | 空 `relations` 列表 → `degraded: true`；不阻断定时子图 |
...
evidence_builder.build_evidence_package()
        ...
        │
        └─► relation_chain.discover_relations()
                  ├─ prompts.get_prompt("relation")
                  ├─ llm_manager.invoke_structured → RelationChainOutput
                  └─ 由 analysis/graph_scheduled.analyze_relations 节点调用
...
| 链 I/O 模型 | `langchain/chain_schemas.py` | 禁止在 `schemas/*.py` 重复定义 ReportChainOutput / DiagnosisChainOutput / RelationChainOutput |
...
| 关系发现 | 跨维度隐藏关系推断 | M7 `discover_relations` 已实现 LLM + 空列表降级 | M7-09 补单测 |
...
| 2026-06-22 | M3-02：五类 Prompt 模板 | `prompts.py` | report/diagnosis/evidence_summary/alert 可用 | relation 于 M7-01 落地 |
...
| 2026-06-22 | M3-05：链 I/O 模型 | `chain_schemas.py` | ReportChainOutput/DiagnosisChainOutput 可校验 | Relation 模型于 M7-02 补充 |
...
| 2026-06-22 | M3-09：文档收敛，标记 M3 主链路完成 | `DEV.md` | 七文件状态更新为已实现；降级策略与依赖关系已记录 | — |
| 2026-06-22 | M6-01：alert_chain 实现 | `alert_chain.py` | explain_alert LLM + 模板降级；ok/degraded/title/detail | — |
| 2026-06-22 | M6-07：文档收敛（M6 alert 部分） | `DEV.md` | alert_chain 标已实现；降级策略与主图调用关系已记录 | — |
| 2026-06-22 | M7-01：RELATION_PROMPT 落地 | `prompts.py` | relation 模板可经 get_prompt 获取 | — |
| 2026-06-22 | M7-02：RelationChainOutput 落地 | `chain_schemas.py` | RelationItem + RelationChainOutput；confidence 钳制 | — |
| 2026-06-22 | M7-03：relation_chain 实装 | `relation_chain.py` | discover_relations LLM 结构化 + 空列表降级 | — |
| 2026-06-22 | **M7-10：文档收口** | `DEV.md` | relation_chain 标已实现；RELATION_PROMPT/RelationChainOutput 契约与降级已记录 | 单测见 M7-09 |
