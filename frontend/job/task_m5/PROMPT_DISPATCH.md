...
唯一修改：`src/api/diagnosis.js`、`src/api/analysis.js` + `STATUS.md`
...
依赖：F5-01
唯一修改：`DiagnosisEntryPanel.vue` + `STATUS.md`
```
...
唯一修改：`ConclusionPanel.vue` + `STATUS.md`
要点：仪表先于文字；禁止聊天气泡
```
...
唯一修改：`EvidenceTimeline.vue` + `ServiceTopology.vue` + `STATUS.md`
```
...
唯一修改：`SuggestionChecklist.vue` + `DiagnosisStageRing.vue` + `STATUS.md`
...
依赖：F5-02~05
唯一修改：`views/analysis/diagnosis.vue` + `STATUS.md`
```
...
唯一修改：`TraceSearchBar.vue` + `TraceWaterfall.vue` + `STATUS.md`
```
...
依赖：F5-07
唯一修改：`views/analysis/trace.vue` + `STATUS.md`
```
